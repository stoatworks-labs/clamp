...
`demo/plugin.js` holds four GLSL fragments and so does `source/Shaders.cpp`.
...
Nothing else can. `cltest` drives the real plugin class through a real FFGL
...
Nothing here checks the *ported* half. makeTimeline, walkField, sagStep, makeTriode, the Clock and every conversion
in plugin.js are a hand translation of Model.cpp, Clock.cpp, Controls.cpp and
Clamp::ProcessOpenGL, and only a reader can tell
...
SHADERS = [
    ...
    ("SUMS", "source/Shaders.cpp", "kSums"),
    ("FILL", "source/Shaders.cpp", "kFill"),
