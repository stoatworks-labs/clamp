...
test card and reports any that made no difference at all.
    ...
    python3 tools/sweep.py [--binary build/cltest] [--size WxH] [--jobs N] [--allow-no-gl]

Exit code 1 means something is dead.

------------------------------------------------------------------ the traps

**The card has to change its level.** A coupling capacitor does nothing to a
picture that never changes but shift it once; the sag attack and recovery do
nothing at all to a constant APL. `cltest --out` renders the moving card --
a block that cuts between black and white every 0.2 s, colour bars, a ramp
-- for 40 frames at 60 fps, which is two rises and two falls of the block.

**Some controls need another to mean anything.** Bias and Drive shape the
triode, which is off by default; Clamp Reference is where a clamp pulls,
which the default clamp barely does. CONTEXT says what else has to be true
for each, and uses nothing but the plugin's own parameters.

**An option's range is its element count.** An option parameter's range reads
back 0..1 whatever its element count, so `cltest --list` prints the count as
...
CONTEXT = {
    "Clamp Reference": {"Clamp Health": 0.8},
    "Bias": {"Triode On": 1},
    "Drive": {"Triode On": 1},
    "Sag Attack": {"Sag Depth": 1},
    "Sag Recovery": {"Sag Depth": 1},
}
...
def main():
    ...
    ap.add_argument("--binary", default=str(ROOT / "build" / "cltest"))
    ...
    scratch = tempfile.mkdtemp(prefix="clsweep")

    # One render first: a runner with no GL at all cannot sweep anything, and
    # that has to read as a skip, loudly, not as thirteen dead controls.
