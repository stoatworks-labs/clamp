...
#                 strings `cltest --dump-shaders` writes.
...
#                   --model     the plugin's timing against BT.470 / SMPTE
#                               170M, its control laws, its blocked walk
#                               against a serial per-sample one, the field
#                               map and its fixed point, the supply's step
#                   --clock     a six-day millisecond host clock decides the
#                               same fields as a fresh seconds one and as
#                               exact arithmetic
#                   --names     nothing a host will silently truncate; the
#                               host reads SW Clamp / CL01 / effect
#   physics       every rendering check, at TWO rasters: 320x180, which is
#                 what CI renders at, and 1280x720. Each is measured out of
#                 the picture:
#                   --droop     after a burst, black falls back on
#                               exp( -t / tau ) with blanking counted, at tau
#                               of a line, a field and a second
#                   --tilt      each line of a flat field tilts by
#                               1 - exp( -T_active / tau )
#                   --apl       black shifts with APL by the closed form;
#                               the primed state is the settled one
#                   --clamp     the residual after each porch is
#                               exp( -T_porch / tau_c ); black settles at the
#                               reference
#                   --reference every pixel against a serial double run
#                   --continuity six host rates, one trajectory
#                   --sag       the supply's attack and recovery
#                   --identity  a perfect clamp returns the input
...
step "shaders"
if out=$(tools/check-shaders.sh "$CLTEST" 2>&1); then
	pass "$( printf '%s\n' "$out" | tail -1 | sed 's/^ *//' )"
else
...
# working, and they stop being the same effect. Character for character --
# reformatting counts. It says nothing about the page's PORT of the CPU half;
# only a reader checks that.
...
for size in 320x180 1280x720; do
	...
	for check in droop tilt apl clamp reference continuity sag identity resize negative; do
		if out=$("$CLTEST" --$check --size $size 2>&1); then
			pass "cltest --$check: $( printf '%s\n' "$out" | grep -v '^$' | grep -v 'checks,' | tail -1 )"
		else
			fail "cltest --$check at $size"
			printf '%s\n' "$out" | sed 's/^/      /'
		fi
	done
done
...
got=$( "$CLTEST" --pipe --size 64x36 < "$raw" 2>/dev/null | wc -c | tr -d ' ' )
status=${PIPESTATUS[0]}
...
# pipeline whatever cltest did, and the refusal would pass for the wrong reason.
printf '0 No Such Control 0.5\n' > "$cues"
"$CLTEST" --pipe --size 64x36 --script "$cues" < "$raw" >/dev/null 2>&1
status=$?
if [ "$status" -eq 2 ]; then
...
got=$( "$CLTEST" --pipe --size 64x36 --fail-render-at 1 < "$raw" 2>/dev/null | wc -c | tr -d ' ' )
status=${PIPESTATUS[0]}
...
"$CLTEST" --pipe --size 64x36 < "$many" 2>/dev/null | head -c 100 >/dev/null
status=${PIPESTATUS[0]}
if [ "$status" -eq 1 ]; then
...
step "sweep"
if out=$(python3 tools/sweep.py --binary "$CLTEST" 2>/dev/null); then
	pass "$( printf '%s\n' "$out" | tail -1 )"
else
...
"$CLTEST" --bench --frames 60 2>&1 | sed -n '3,6p' | sed 's/^/   /'

BUNDLE="$BUILD/Clamp.bundle"
BIN="$BUNDLE/Contents/MacOS/Clamp"
...
if [ "$(uname)" = "Darwin" ] && [ -d "$BUNDLE" ]; then
	...
	if [ "$ident" = "com.stoatworks.ffgl.clamp" ]; then
	...
	if codesign --force --sign - --timestamp=none "$tmp/Clamp.bundle" >/dev/null 2>&1; then
	...
	if [ -x "$OXBOW" ]; then
		...
		for want in "name:        SW Clamp" "id:          CL01" "type:        effect"; do
			case "$probe" in
