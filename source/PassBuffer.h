...
/**
    ...
    **It reallocates only when it has to.** Ensure() is called every frame and
    ...
    host's picture changes size or the Standard changes its line count. A
    reallocation clears, which is safe here because nothing on the GPU
    outlives a frame: the capacitor's state is a handful of doubles on the
    CPU (see `Clamp.h` and `cltest --resize`).
    ...
    **It owns its filtering.** Every buffer in this plugin is samples of a
    scan line read texel for texel with `texelFetch` -- a sample is a moment
    on the raster's timeline, and a filtered read between two is a read
    between two moments -- so every buffer here is Nearest. Linear and
    Mipmapped are tinsel's and are unused.

    Copied from tinsel (by way of standards), where it was written for a glow
    chain; the mechanism is unchanged.
*/
