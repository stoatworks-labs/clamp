...
/**
    ...
    **A shader that will not compile.** `InitGL` returns `FF_FAIL` and from
    ...
    message anywhere; with six passes this also records *which* one, which is
    ...
    **A field store that could not be allocated.** This plugin holds two
    host frames on the source standard's lines, eight source fields and four
    destination fields, all as wide as the host's picture, and the failure
    mode when the driver says no is a black frame with nothing to explain it.
    What goes in the log is the width asked for: at 4K the store comes to
    about 135 MB of half-float texture.
*/
namespace clampfx::diag
{
